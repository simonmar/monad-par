...
alertNonWorker (c, a) = 
    let go [] = Nothing
        go ((i, Idle b):_) = Just (i, Idle b)
        go ((i, Absent):_) = Just (i, Absent)
        go (_:rest) = go rest
        ret = go (assocs a)
    in (maybe (c, a) (\(i, _) -> (c-1, a // [(i, Alerted)])) ret, ret)
...
alertThisNonWorker n (c, a) = case a!n of
    Idle b -> ((c-1, a // [(n, Alerted)]), Idle b)
    Absent -> ((c-1, a // [(n, Alerted)]), Absent)
    s      -> ((c, a), s)
...
setMeIdle n m (c, a) =
    ((if isAlertable (a!n) then c else c+1,
      a // [(n, Idle m)]),
...
setMeWorking n (c, a) =
    ((if isAlertable (a!n) then c-1 else c, a // [(n, Working)]), ())
...
updateMyWork n newWork =
    atomicModifyIORef currentWork (\a -> (a // [(n, newWork)], ()))
